...
IFS="$(echo -e "\n\r")"
for LINE in `cat /usr/local/deployment/onix1/config/post_install.properties`
do
  case $LINE in
    \#*) ;;
    *)
...
nohup java -Dconfig=/usr/local/deployment/onix1/config/post_install.properties $SERVICE_JVMARGS -jar $JAR_NAME > $LOG_FILE 2> $ERR_FILE < /dev/null &
